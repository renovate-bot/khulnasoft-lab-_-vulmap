...
import (
	...
	"github.com/khulnasoft-lab/vulmap/pkg/core/inputs/hybrid"
	...
	"github.com/khulnasoft-lab/vulmap/pkg/protocols/common/automaticscan"
...
type Runner struct {
	...
	cloudClient       *vulmapcloud.Client
	cloudTargets      []string
}
...
func New(options *types.Options) (*Runner, error) {
	...
	if options.Cloud {
		runner.cloudClient = vulmapcloud.New(options.CloudURL, options.CloudAPIKey)
	}

	//  Version check by default
	if config.DefaultConfig.CanCheckForUpdates() {
		if err := installer.VulmapVersionCheck(); err != nil {
			if options.Verbose || options.Debug {
				gologger.Error().Msgf("vulmap version check failed got: %s\n", err)
			}
		}
		...
		if err := tm.FreshInstallIfNotExists(); err != nil {
			gologger.Warning().Msgf("failed to install vulmap templates: %s\n", err)
		}
		if err := tm.UpdateIfOutdated(); err != nil {
			gologger.Warning().Msgf("failed to update vulmap templates: %s\n", err)
		}

		if config.DefaultConfig.NeedsIgnoreFileUpdate() {
			if err := installer.UpdateIgnoreFile(); err != nil {
				gologger.Warning().Msgf("failed to update vulmap ignore file: %s\n", err)
			}
		}
		...
		if options.UpdateTemplates {
			...
			if !config.DefaultConfig.NeedsTemplateUpdate() {
				gologger.Info().Msgf("No new updates found for vulmap templates")
			}
	...
	hmapInput, err := hybrid.New(&hybrid.Options{
		...
		NotFoundCallback: func(target string) bool {
			...
			if parseErr != nil {
				if err := runner.cloudClient.ExistsDataSourceItem(vulmapcloud.ExistsDataSourceItemRequest{Contents: target, Type: "targets"}); err == nil {
...
// RunEnumeration sets up the input layer for giving input vulmap.
...
func (r *Runner) RunEnumeration() error {
	...
	// stage by the vulmap engine modules.
	...
	if r.options.Cloud {
		...
		store.NotFoundCallback = func(template string) bool {
			...
			if parseErr != nil {
				if err := r.cloudClient.ExistsDataSourceItem(vulmapcloud.ExistsDataSourceItemRequest{Type: "templates", Contents: template}); err == nil {
...
// displayExecutionInfo displays misc info about the vulmap engine execution
func (r *Runner) displayExecutionInfo(store *loader.Store) {
	...
	gologger.Info().Msgf("Current vulmap version: %v %v", config.Version, updateutils.GetVersionDescription(config.Version, cfg.LatestVulmapVersion))
	gologger.Info().Msgf("Current vulmap-templates version: %v %v", cfg.TemplateVersion, updateutils.GetVersionDescription(cfg.TemplateVersion, cfg.LatestVulmapTemplatesVersion))
