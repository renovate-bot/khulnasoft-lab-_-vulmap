...
func (r *Runner) getScanList(limit int) error {
	...
	for {
		...
		for _, v := range items {
			...
			res := vulmapcloud.PrepareScanListOutput(v)
			if r.options.JSONL {
...
func (r *Runner) listDatasources() error {
	...
	for _, source := range datasources {
		if r.options.JSONL {
			...
			values = append(values, []string{strconv.FormatInt(source.ID, 10), source.Updatedat.Format(vulmapcloud.DDMMYYYYhhmmss), source.Type, source.Repo, source.Path})
		} else {
			gologger.Silent().Msgf("%d. [%s] [%s] [%s] %s", source.ID, source.Updatedat.Format(vulmapcloud.DDMMYYYYhhmmss), source.Type, source.Repo, source.Path)
		}
	}
	if !r.options.NoTables {
		r.prettyPrintTable(header, values)
...
func (r *Runner) processDataSourceItem(repo, token, Type string) (int64, error) {
	ID, err := r.cloudClient.StatusDataSource(vulmapcloud.StatusDataSourceRequest{Repo: repo, Token: token})
	if err != nil {
		...
		resp, err := r.cloudClient.AddDataSource(vulmapcloud.AddDataSourceRequest{Type: Type, Repo: repo, Token: token})
		if err != nil {
...
func (r *Runner) addCloudReportingSource() error {
	...
	if rcOptions.Jira != nil {
		...
		requestObj := vulmapcloud.AddReportingSourceRequest{
			Type:    "jira",
			Payload: payload,
		}
