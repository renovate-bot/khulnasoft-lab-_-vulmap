...
func (r *Runner) runCloudEnumeration(store *loader.Store, cloudTemplates, cloudTargets []string, nostore bool, limit int) (*atomic.Bool, error) {
	...
	catalogChecksums := vulmapcloud.ReadCatalogChecksum()
	...
	taskID, err := r.cloudClient.AddScan(&vulmapcloud.AddScanRequest{
		RawTargets:       targets,
...
func getTemplateRelativePath(templatePath string) string {
	splitted := strings.SplitN(templatePath, "vulmap-templates", 2)
	if len(splitted) < 2 {
		return ""
	}
...
func getCloudFilteringFromOptions(options *types.Options) *vulmapcloud.AddScanRequestConfiguration {
	return &vulmapcloud.AddScanRequestConfiguration{
