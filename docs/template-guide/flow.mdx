...
Template flow engine introduced in v3, brings two significant enhancements to Vulmap:
...
With addition of flow in Vulmap v3 we can re-write this template to first check if target is a wordpress site, if yes then bruteforce login with default credentials and this can be achieved by simply adding one line of content  i.e `flow: http(1) && http(2)` and vulmap will take care of everything else.
...
Flow is a powerful Vulmap feature that provides enhanced orchestration capabilities for executing requests. The simplicity of conditional execution is just the beginning. With ﻿flow, you can:
...
Think of request execution orchestration as a bridge between JavaScript and Vulmap, offering two-way interaction within a specific template.
...
In this code, we've introduced 5 extra lines of JavaScript. This allows the template to perform vhost enumeration. The best part? You can run this at scale with all features of Vulmap, using supported inputs like ﻿ASN, ﻿CIDR, ﻿URL.
...
By understanding and taking advantage of Vulmap's `flow`, you can redefine the way you orchestrate request executions, making your templates much more powerful and efficient.
...
In vulmap, any listed protocol can be invoked or executed in JavaScript using the `protocol_name()` format. For example, you can use `http()`, `dns()`, `ssl()`, etc.

If you want to execute a specific request of a protocol (refer to vulmap-flow-dns for an example), it can be achieved by passing either:
...
Iterate is a vulmap js helper function which can be used to iterate over any type of value like **array**, **map**, **string**, **number** while handling empty/nil values.
