...
Another useful tag to always add in `info` block is **tags**. This allows you to set some custom tags to a template, depending on the purpose like `cve`, `rce` etc. This allows vulmap to identify templates with your input tags and only run them.
...
It's possible to add metadata nodes, for example, to integrates with [uncover](https://github.com/khulnasoft-lab/uncover) (cf. [Uncover Integration](https://vulmap.khulnasoft-lab.io/vulmap/get-started/#uncover-integration)).

The metadata nodes are crafted this way: `<engine>-query: '<query>'` where:

- `<engine>` is the search engine, equivalent of the value of the `-ue` option of vulmap or the `-e` option of uncover
- `<query>` is the search query, equivalent of the value of the `-uq` option of vulmap or the `-q` option of uncover
