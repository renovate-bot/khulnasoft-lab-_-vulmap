...
Vulmap templates can be primarily executed in two ways:

1. **Templates** (`-t/templates`)

As default, all the templates (except vulmap-ignore list) get executed from the default template installation path.

```sh
vulmap -u https://example.com
```

Custom template directory or multiple template directory can be executed as follows:

```sh
vulmap -u https://example.com -t cves/ -t exposures/
```

Custom template Github repos are downloaded under `github` directory. Custom repo templates can be passed as follows:

```sh
vulmap -u https://example.com -t github/private-repo
```

Similarly, Templates can be executed against a list of URLs.

```sh
vulmap -list http_urls.txt
```

2. **Workflows** (`-w/workflows`)

```sh
vulmap -u https://example.com -w workflows/
```

Similarly, Workflows can be executed against a list of URLs.

```sh
vulmap -list http_urls.txt -w workflows/wordpress-workflow.yaml
```

### Vulmap **Filters**

Vulmap engine supports three basic filters to customize template execution.
...
For example, below command will run all the templates installed at `~/vulmap-templates/` directory and has `cve` tags in it.

```sh
vulmap -u https://example.com -tags cve
```

And this example will run all the templates available under `~/vulmap-templates/exposures/` directory and has `config` tag in it.

```sh
vulmap -u https://example.com -tags config -t exposures/
```
...
```sh
vulmap -u https://example.com -tags cve -severity critical,high -author geeknik
```
...
```sh
vulmap -tc "contains(id,'xss') || contains(tags,'xss')"
vulmap -tc "contains(tags,'cve') && contains(tags,'ssrf')"
vulmap -tc "contains(name, 'Local File Inclusion')"
```
...
```sh
vulmap -w workflows/wordpress-workflow.yaml -severity critical,high -list http_urls.txt
```
...
In Workflows, Vulmap filters are applied on templates or sub-templates running via workflows, not on the workflows itself.

</Note>

### Public Templates

Vulmap has built-in support for automatic template download/update from [**vulmap templates**](https://github.com/khulnasoft-lab/vulmap-templates) project which provides [community-contributed](https://github.com/khulnasoft-lab/vulmap-templates#-community) list of ready-to-use templates that is constantly updated.

Vulmap checks for new community template releases upon each execution and automatically downloads the latest version when available. optionally, this feature can be disabled using the `-duc` cli flag or the configuration file.

### Custom Templates

Users can create custom templates on a personal public / private GitHub / AWS Bucket that they wish to run / update while using vulmap from any environment without manually downloading the GitHub repository everywhere.
...
<AccordionGroup>
  <Accordion title="For GitHub Project" icon="pencil">
    ...
    export GITHUB_TEMPLATE_REPO=my_vulmap_template
    ```
...
```bash
# Disable download from the default vulmap-templates project
export DISABLE_VULMAP_TEMPLATES_PUBLIC_DOWNLOAD=true

# Disable download from public / private GitHub project(s)
export DISABLE_VULMAP_TEMPLATES_GITHUB_DOWNLOAD=true

# Disable download from public / private GitLab project(s)
export DISABLE_VULMAP_TEMPLATES_GITLAB_DOWNLOAD=true

# Disable download from public / private AWS Bucket(s)
export DISABLE_VULMAP_TEMPLATES_AWS_DOWNLOAD=true

# Disable download from public / private Azure Blob Storage
export DISABLE_VULMAP_TEMPLATES_AZURE_DOWNLOAD=true
```
...
This command will clone the repository containing the custom templates to the default vulmap templates directory (`$HOME/vulmap-templates/github/`).

The directory structure of the custom templates looks as follows:

```bash
tree $HOME/vulmap-templates/
...
```
vulmap -t github/my_custom_template -u https://example.com
```

The vulmap engine can be updated to latest version by using the `-update` flag.
...
```bash
Vulmap is a fast, template based vulnerability scanner focusing
...
TEMPLATES:
   -nt, -new-templates                    run only new templates added in latest vulmap-templates release
   ...
   -validate                              validate the passed templates to vulmap
...
OUTPUT:
   ...
   -sresp, -store-resp           store all request/response passed through vulmap to output directory
   -srd, -store-resp-dir string  store all request/response passed through vulmap to custom directory (default "output")
   ...
   -rdb, -report-db string       vulmap reporting database (always use this to persist report data)
   -ms, -matcher-status          display match failure status
   -me, -markdown-export string  directory to export results in markdown format
   -se, -sarif-export string     file to export results in SARIF format

CONFIGURATIONS:
   -config string                 path to the vulmap configuration file
   -fr, -follow-redirects         enable following redirects for http templates
   -fhr, -follow-host-redirects   follow redirects on the same host
   -mr, -max-redirects int        max number of redirects to follow for http templates (default 10)
   -dr, -disable-redirects        disable redirects for http templates
   -rc, -report-config string     vulmap reporting module configuration file
   -H, -header string[]           custom header/cookie to include in all http request in header:value format (cli, file)
   -V, -var value                 custom vars in key=value format
   -r, -resolvers string          file containing resolver list for vulmap
   ...
   -sandbox                       sandbox vulmap for safe templates execution
...
HEADLESS:
   ...
   -sc, -system-chrome          use local installed Chrome browser instead of vulmap installed
...
DEBUG:
   ...
   -version                  show vulmap version
   -hm, -hang-monitor        enable vulmap hang monitoring
   -v, -verbose              show verbose output
   -profile-mem string       optional vulmap memory profile dump file
   -vv                       display templates loaded for scan
   -svd, -show-var-dump      show variables dump for debugging
   -ep, -enable-pprof        enable pprof debugging server
   -tv, -templates-version   shows the version of the installed vulmap-templates
   -hc, -health-check        run diagnostic check up

UPDATE:
   -un, -update                      update vulmap engine to the latest released version
   -ut, -update-templates            update vulmap-templates to latest released version
   -ud, -update-template-dir string  custom directory to install / update vulmap-templates
   -duc, -disable-update-check       disable automatic vulmap/templates update check

STATISTICS:
   -stats                    display statistics about the running scan
   -sj, -stats-json          write statistics data to an output file in JSONL(ines) format
   -si, -stats-interval int  number of seconds to wait between showing a statistics update (default 5)
   -mp, -metrics-port int    port to expose vulmap metrics on (default 9092)
```

<Tip>
  From Vulmap v3.0.0 `-metrics` port has been removed and merged with `-stats`
...
Vulmap have multiple rate limit controls for multiple factors, including a number of templates to execute in parallel, a number of hosts to be scanned in parallel for each template, and the global number of request / per second you wanted to make/limit using vulmap, here is an example of each flag with description.
...
Feel free to play with these flags to tune your vulmap scan speed and accuracy.
...
Many BugBounty platform/programs requires you to identify the HTTP traffic you make, this can be achieved by setting custom header using config file at `$HOME/.config/vulmap/config.yaml` or CLI flag `-H / header`
...
header:
  ...
  - 'User-Agent: Mozilla/5.0 (Windows NT 10.0; WOW64) / vulmap'
```
...
```yaml
vulmap -header 'User-Agent: Mozilla/5.0 (Windows NT 10.0; WOW64) / vulmap' -list urls.txt -tags cves
```
...
Vulmap supports a variety of methods for excluding / blocking templates from execution. By default, **vulmap** excludes the tags/templates listed below from execution to avoid unexpected fuzz based scans and some that are not supposed to run for mass scan, and these can be easily overwritten with vulmap configuration file / flags.

- Default [Template ignore](https://github.com/khulnasoft-lab/vulmap-templates/blob/master/.vulmap-ignore) list.

<Warning>
  **vulmap-ignore** file is not supposed to be updated / edited / removed by
  user, to overwrite default ignore list, utilize [vulmap
  configuration](/getting-started/features#vulmap-config) file.{' '}
</Warning>

Vulmap engine supports two ways to manually exclude templates from scan,
...
```
vulmap -list urls.txt -t cves/ -exclude-templates cves/2020/CVE-2020-XXXX.yaml
```
...
```
vulmap -list urls.txt -exclude-templates exposed-panels/ -exclude-templates technologies/
```
...
```
vulmap -l urls.txt -t cves/ -etags xss
```
...
```
vulmap -l urls.txt -t cves/ -etags sqli,rce
```

</Note>

To easily overwrite [vulmap-ignore](https://github.com/khulnasoft-lab/vulmap-templates/blob/master/.vulmap-ignore), Vulmap engine supports **include-tags** / **include-templates** flag.

<Note>
Example of running blocked templates

```
vulmap -l urls.txt -include-tags iot,misc,fuzz
```
...
Vulmap supports integration with [uncover module](https://github.com/khulnasoft-lab/uncover) that supports services like Shodan, Censys, Hunter, Zoomeye, many more to execute Vulmap on these databases.
...
```
export SHODAN_API_KEY=xxx
vulmap -id 'CVE-2021-26855' -uq 'vuln:CVE-2021-26855' -ue shodan
```
...
Template snippet of [CVE-2021-26855](https://github.com/khulnasoft-lab/vulmap-templates/blob/master/cves/2021/CVE-2021-26855.yaml)
...
```console
vulmap -t cves/2021/CVE-2021-26855.yaml -uncover
vulmap -tags cve -uncover
```

We can update the vulmap configuration file to include these tags for all scans.

## **Large scans** using Vulmap

Vulmap fully utilises resources to optimise scanning speed. However, when scanning **thousands**, if not **millions, of targets**, scanning using default parameter values is bound to cause some performance issues ex: low RPS, Slow Scans, Process Killed, High RAM consumption, etc. this is due to limited resources and network I/O. Hence following parameters need to be tuned based on system configuration and targets.

For enterprises dealing with large-scale scanning, optimizing Vulmap can be a burdensome task, especially when scans change frequently. That's where [Vulmap Enterprise](https://nuclei.sh/enterprise) comes in. With its managed offering and dedicated support, Vulmap Enterprise minimizes the burden of optimizing Vulmap on large scans, making it an ideal choice for enterprise-level scanning needs.
...
> Since release of [v2.3.2](https://blog.khulnasoft-lab.io/vulmap-v2-3-0-release/) vulmap uses [goflags](https://github.com/khulnasoft-lab/goflags) for clean CLI experience and long/short formatted flags.
>
> [goflags](https://github.com/khulnasoft-lab/goflags) comes with auto-generated config file support that coverts all available CLI flags into config file, basically you can define all CLI flags into config file to avoid repetitive CLI flags that loads as default for every scan of vulmap.
>
> Default path of vulmap config file is `$HOME/.config/vulmap/config.yaml`, uncomment and configure the flags you wish to run as default.
...
# Template Allowlist
include-tags: dos,fuzz # Tag based inclusion (allows overwriting vulmap-ignore list)
include-templates: # Template based inclusion (allows overwriting vulmap-ignore list)
  - vulnerabilities/xxx
...
<Note>
**Running vulmap with custom config file**

    ```
    vulmap -config project.yaml -list urls.txt
    ```
...
Vulmap comes with reporting module support with the release of [v2.3.0](https://vulmap.khulnasoft-lab.io/releases/vulmap-changelog/#vulmap-v230-10-march-2021) supporting GitHub, GitLab, and Jira integration, this allows vulmap engine to create automatic tickets on the supported platform based on found results.
...
`-rc, -report-config` flag can be used to provide a config file to read configuration details of the platform to integrate. Here is an [example config file](https://github.com/khulnasoft-lab/vulmap/blob/main/cmd/vulmap/issue-tracker-config.yaml) for all supported platforms.
...
The Jira reporting options allows for custom fields, as well as using variables from the Vulmap templates in those custom fields.
...
**Running vulmap with reporting module:**

```bash
vulmap -l urls.txt -t cves/ -rc issue-tracker.yaml
```
...
```bash
vulmap -l urls.txt -t cves/ -rc issue-tracker.yaml -rdb prod
```

**<ins>Markdown Export</ins>**

Vulmap supports markdown export of valid findings with `-me, -markdown-export` flag, this flag takes directory as input to store markdown formatted reports.
...
```bash
vulmap -l urls.txt -t cves/ -irr -markdown-export reports
```

**<ins>SARIF Export</ins>**

Vulmap supports SARIF export of valid findings with `-se, -sarif-export` flag. This flag takes a file as input to store SARIF formatted report.

```bash
vulmap -l urls.txt -t cves/ -sarif-export report.sarif
```

It is also possible to visualize Vulmap results using **SARIF** files.
...
More info on the SARIF output is documented [here](https://github.com/khulnasoft-lab/vulmap/pull/2925).

<Note>
  These are **not official** viewers of Vulmap and `Vulmap` has no liability
  towards any of these options to visualize **Vulmap** results. These are just
  some publicly available options to visualize SARIF files.
</Note>

## Scan **Metrics**

Vulmap expose running scan metrics on a local port `9092` when `-metrics` flag is used and can be accessed at **localhost:9092/metrics**, default port to expose scan information is configurable using `-metrics-port` flag.

Here is an example to query `metrics` while running vulmap as following `vulmap -t cves/ -l urls.txt -metrics`
...
Vulmap engine supports passive mode scanning for HTTP based template utilizing file support, with this support we can run HTTP based templates against locally stored HTTP response data collected from any other tool.

```sh
vulmap -passive -target http_data
```

<Note>Passive mode support is limited for templates having `{{BasedURL}}` or `{{BasedURL/}}` as base path.</Note>

## Running With Docker
If Vulmap was installed within a Docker container based on the [installation instructions](./install),
...
### Basic Usage
This example runs a Vulmap container against `google.com`, prints the results to JSON and removes the container once it
has completed:
```sh
docker run --rm khulnasoft-lab/vulmap -u google.com -jsonl
```

### Using Volumes
This example runs a Vulmap container against a list of URLs, writes the results to a `.jsonl` file and removes the
container once it has completed.
```sh
# This assumes there's a file called `urls.txt` in the current directory
docker run --rm -v ./:/app/ khulnasoft-lab/vulmap -l /app/urls.txt -jsonl /app/results.jsonl
