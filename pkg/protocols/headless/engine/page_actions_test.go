...
import (
	...
	"github.com/khulnasoft-lab/vulmap/pkg/testutils/testheadless"
...
func TestActionNavigate(t *testing.T) {
	...
	testHeadlessSimpleResponse(t, response, actions, 20*time.Second, func(page *Page, err error, out map[string]string) {
		...
		require.Equal(t, "Vulmap Test Page", page.Page().MustInfo().Title, "could not navigate correctly")
	})
}
...
func TestActionGetResource(t *testing.T) {
	response := `
		<html>
			...
			<body>
				<img id="test" src="https://raw.githubusercontent.com/khulnasoft-lab/wallpapers/main/pd-floppy.jpg">
			</body>
		</html>`
