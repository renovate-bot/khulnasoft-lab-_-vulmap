...
func TestRemoteTemplates(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				config: &Config{
					TemplateURLs:             []string{"https://raw.githubusercontent.com/khulnasoft-lab/vulmap-templates/main/technologies/tech-detect.yaml"},
					RemoteTemplateDomainList: []string{"localhost", "raw.githubusercontent.com"},
					Catalog:                  catalog,
				},
			},
			want: &Store{
				finalTemplates: []string{"https://raw.githubusercontent.com/khulnasoft-lab/vulmap-templates/main/technologies/tech-detect.yaml"},
			},
			wantErr: false,
		},
		{
