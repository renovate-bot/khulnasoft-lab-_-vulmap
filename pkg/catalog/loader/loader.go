...
import (
	...
	cfg "github.com/khulnasoft-lab/vulmap/pkg/catalog/config"
...
var (
	TrustedTemplateDomains = []string{"templates.vulmap.sh", "cloud.khulnasoft-lab.io"}
)
...
// Store is a storage for loaded vulmap templates
type Store struct {
...
func handleTemplatesEditorURLs(input string) string {
	...
	if !strings.HasSuffix(parsed.Hostname(), "templates.vulmap.sh") {
		return input
	}
...
// the complete compiled templates for a vulmap execution configuration.
