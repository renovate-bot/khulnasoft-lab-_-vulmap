...
// Options contains the configuration options for vulmap scanner.
type Options struct {
	...
	// Workflows specifies any workflows to run by vulmap
	...
	// ProjectPath allows vulmap to use a user defined project folder
	ProjectPath string
	...
	ReportingDB string
	// ReportingConfig is the config file for vulmap reporting module
	...
	SarifExport string
	// CloudURL is the URL for the vulmap cloud endpoint
	CloudURL string
	// CloudAPIKey is the api-key for the vulmap cloud endpoint
	CloudAPIKey string
	...
	ScanOutput string
	// ResolversFile is a file containing resolvers for vulmap.
	...
	// SystemResolvers enables override of vulmap's DNS client opting to use system resolver stack.
	...
	Silent bool
	// Validate validates the templates passed to vulmap.
	Validate bool
	// NoStrictSyntax disables strict syntax check on vulmap templates (allows custom key-value pairs).
	NoStrictSyntax bool
	...
	JSONLExport string
	// Cloud enables vulmap cloud scan execution
	Cloud bool
	...
	TemplateList bool
	// HangMonitor enables vulmap hang monitoring
	HangMonitor bool
	...
	// PublicTemplateDisableDownload disables downloading templates from the vulmap-templates public repository
...
// DefaultOptions returns default options for vulmap
...
func (o *Options) GetValidAbsPath(helperFilePath, templatePath string) (string, error) {
	...
	// 1. If helper file is present in vulmap-templates directory
	...
	if err == nil {
		// As per rule 1, if helper file is present in vulmap-templates directory, allow it
