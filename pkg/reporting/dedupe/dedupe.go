// Package dedupe implements deduplication layer for vulmap-generated
// issues.
//
...
// Storage is a duplicate detecting storage for vulmap scan events.
type Storage struct {
	temporary string
	storage   *leveldb.DB
}

// New creates a new duplicate detecting storage for vulmap scan events.
func New(dbPath string) (*Storage, error) {
	storage := &Storage{}

	var err error
	if dbPath == "" {
		dbPath, err = os.MkdirTemp("", "vulmap-report-*")
