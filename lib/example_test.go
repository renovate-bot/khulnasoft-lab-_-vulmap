...
// A very simple example on how to use vulmap engine
func ExampleVulmapEngine() {
	// create vulmap engine with options
	ne, err := vulmap.NewVulmapEngine(
		vulmap.WithTemplateFilters(vulmap.TemplateFilters{IDs: []string{"self-signed-ssl"}}), // only run self-signed-ssl template
	)
	if err != nil {
		panic(err)
	}
...
func ExampleThreadSafeVulmapEngine() {
	// create vulmap engine with options
	...
	go func() {
		...
		err = ne.ExecuteVulmapWithOpts([]string{"scanme.sh"},
			vulmap.WithTemplateFilters(vulmap.TemplateFilters{IDs: []string{"nameserver-fingerprint"}}), // only run self-signed-ssl template
		)
		if err != nil {
			panic(err)
		}
