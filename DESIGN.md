# Vulmap Architecture Document

A brief overview of Vulmap Engine architecture. This document will be kept updated as the engine progresses.
...
Model package implements Information structure for Vulmap Templates. `Info` contains all major metadata information for the template. `Classification` structure can also be used to provide additional context to vulnerability data.
...
Protocols package implements all the request protocols supported by Vulmap. This includes http, dns, network, headless and file requests as of now. 
...
With this basic premise set, we can now start exploring the current runner implementation which will also walk us through the architecture of vulmap.
...
Output package implements the output writing functionality for Vulmap.

Output Writer implements the Writer interface which is called each time a result is found for vulmap.
...
ResultEvent structure is passed to the Vulmap Output Writer which contains the entire detail of a found result. Various intermediary types like `InternalWrappedEvent` and `InternalEvent` are used throughout vulmap protocols and matchers to describe results in various stages of execution.
...
`HostErrorsCache` is initialised which is used throughout the run of Vulmap enumeration to keep track of errors per host and skip further requests if the errors are greater than the provided threshold. The functionality for the error tracking cache is defined in [hosterrorscache.go](https://github.com/khulnasoft-lab/vulmap/blob/main/pkg/protocols/common/hosterrorscache/hosterrorscache.go) and is pretty simplistic in nature.
...
Operators package implements all the matching and extracting logic of Vulmap. 
...
A protocol only needs to embed the `operators.Operators` type shown above, and it can utilise all the matching/extracting functionality of vulmap.
...
The core of this process is the Execute function which takes an input dictionary as well as a Match and Extract function and return a `Result` structure which is used later during vulmap execution to check for results.
...
Protocols form the core of Vulmap Engine. All the request types like `http`, `dns`, etc. are implemented in form of protocol requests.

A protocol must implement the `Protocol` and `Request` interfaces described above in `pkg/protocols`. We'll take the example of an existing protocol implementation - websocket for this short reference around Vulmap internals.
...
Step by step description of how to add a new protocol to Vulmap - 
...
import (
	...
	"github.com/khulnasoft-lab/vulmap/v3/pkg/protocols/others/websocket"
)
...
That's it, you've added a new protocol to Vulmap. The next good step would be to write integration tests which are described in `integration-tests` and `cmd/integration-tests` directories.
...
```console
$ vulmap -t vulmap-templates/ -u https://example.com -profile-mem mem.pprof
```
...
- [pkg/reporting](./pkg/reporting) - Reporting modules for vulmap.
...
- [pkg/operators](./pkg/operators) - Operators for Vulmap
- [pkg/operators/common/dsl](./pkg/operators/common/dsl) - DSL functions for Vulmap YAML Syntax
...
- [pkg/output](./pkg/output) - Output module for vulmap
