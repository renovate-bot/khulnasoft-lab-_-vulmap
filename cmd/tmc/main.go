...
// allTagsRegex is a list of all tags in vulmap templates except id, info, and -
...
func main() {
	...
	flagSet.SetDescription(`TemplateMan CLI is basic utility built on the TemplateMan API to standardize vulmap templates.`)
	...
	flagSet.CreateGroup("Config", "config",
		flagSet.BoolVarP(&opts.lint, "lint", "l", false, "lint given vulmap template"),
		flagSet.BoolVarP(&opts.validate, "validate", "v", false, "validate given vulmap template"),
		flagSet.BoolVarP(&opts.format, "format", "f", false, "format given vulmap template"),
		flagSet.BoolVarP(&opts.enhance, "enhance", "e", false, "enhance given vulmap template"),
...
func process(opts options) error {
	tempDir, err := os.MkdirTemp("", "vulmap-nvd-%s")
	if err != nil {
		return err
	}
...
// ref: https://github.com/khulnasoft-lab/templateman/blob/main/templateman-rest-api/README.md#enhance-api
