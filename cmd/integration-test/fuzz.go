...
func (h *httpFuzzQuery) Execute(filePath string) error {
	...
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL+"/?id=example", debug)
	if err != nil {
		return err
	}
...
func (h *fuzzModeOverride) Execute(filePath string) error {
	...
	ts := httptest.NewTLSServer(router)
	defer ts.Close()
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL+"/?id=example&name=vulmap", debug, "-fuzzing-mode", "single", "-jsonl")
	if err != nil {
		return err
	}
	...
	if values.Get("name") != "vulmap" {
		return fmt.Errorf("expected fuzzing should not override the name vulmap got %s", values.Get("name"))
	}
	return nil
}
...
func (h *fuzzTypeOverride) Execute(filePath string) error {
	...
	ts := httptest.NewTLSServer(router)
	defer ts.Close()
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL+"?id=example", debug, "-fuzzing-type", "replace", "-jsonl")
	if err != nil {
		return err
	}
	if err = expectResultsCount(results, 1); err != nil {
...
func (h *HeadlessFuzzingQuery) Execute(filePath string) error {
	...
	got, err := testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL+"?url=https://scanme.sh", debug, "-headless")
	if err != nil {
		return err
	}
