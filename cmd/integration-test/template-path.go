...
func (h *cwdTemplateTest) Execute(filePath string) error {
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, "8x8exch02.8x8.com", debug, "-ms")
	if err != nil {
		return err
	}
...
func (h *relativePathTemplateTest) Execute(filePath string) error {
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, "8x8exch02.8x8.com", debug)
	if err != nil {
		return err
	}
	return expectResultsCount(results, 1)
...
func (h *folderPathTemplateTest) Execute(filePath string) error {
	results, err := testutils.RunVulmapBinaryAndGetCombinedOutput(debug, []string{"-t", filePath, "-target", "http://example.com"})
	if err != nil {
		return err
	}
