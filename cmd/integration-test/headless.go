...
func (h *headlessBasic) Execute(filePath string) error {
	...
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL, debug, "-headless")
	if err != nil {
		return err
	}
...
func (h *headlessSelfContained) Execute(filePath string) error {
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, "", debug, "-headless", "-var query=selfcontained")
	if err != nil {
		return err
	}
...
func (h *headlessLocal) Execute(filePath string) error {
	...
	results, err := testutils.RunVulmapWithArgsAndGetResults(debug, args...)
	if err != nil {
		return err
	}
...
func (h *headlessHeaderStatus) Execute(filePath string) error {
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, "https://scanme.sh", debug, "-headless")
	if err != nil {
		return err
	}
