...
func (h *RfcRequestResponse) Execute(filePath string) error {
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, "protocols/offlinehttp/data/", debug, "-passive")
	if err != nil {
		return err
	}
...
func (h *RawRequestResponse) Execute(filePath string) error {
	_, err := testutils.RunVulmapTemplateAndGetResults(filePath, "protocols/offlinehttp/data/", debug, "-passive")
	if err == nil {
