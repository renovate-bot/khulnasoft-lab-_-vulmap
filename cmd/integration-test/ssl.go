...
func (h *sslBasicZtls) Execute(filePath string) error {
	...
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL, debug, "-ztls")
	if err != nil {
		return err
	}
...
func (h *sslWithVars) Execute(filePath string) error {
	...
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL, debug, "-V", "test=asdasdas")
	if err != nil {
		return err
	}
