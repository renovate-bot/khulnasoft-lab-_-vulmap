...
func (h *workflowBasic) Execute(filePath string) error {
	...
	results, err := testutils.RunVulmapWorkflowAndGetResults(filePath, ts.URL, debug)
	if err != nil {
		return err
	}
...
func (h *workflowDnsKeyValueShare) Execute(filePath string) error {
	results, err := testutils.RunVulmapWorkflowAndGetResults(filePath, "http://scanme.sh", debug)
	if err != nil {
		return err
	}
...
func (h *workflowSharedCookies) Execute(filePath string) error {
	...
	_, err := testutils.RunVulmapWorkflowAndGetResults(filePath, ts.URL, debug, "-headless")
	if err != nil {
		return err
	}
