...
func (h *networkBasic) Execute(filePath string) error {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Could not run vulmap: %s\n", err)
		return err
	}
	if routerErr != nil {
...
func (n *networkPort) Execute(filePath string) error {
	...
	results, err = testutils.RunVulmapTemplateAndGetResults(filePath, strings.ReplaceAll(ts.URL, "23846", "443"), debug)
	if err != nil {
		return err
	}
	...
	results, err = testutils.RunVulmapTemplateAndGetResults(filePath, ts2.URL, debug)
	if err != nil {
		return err
	}
