...
func (d *httpDefaultMatcherCondition) Execute(filePath string) error {
	...
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL+"/status", debug)
	if err != nil {
		return err
	}
	...
	results, err = testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL+"/interactsh", debug)
	if err != nil {
		return err
	}
	if routerErr != nil {
...
func (h *httpGetQueryString) Execute(filePath string) error {
	router := httprouter.New()
	router.GET("/", func(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
		if strings.EqualFold(r.URL.Query().Get("test"), "vulmap") {
...
func (h *httpDisableRedirects) Execute(filePath string) error {
	...
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL, debug, "-dr")
	if err != nil {
		return err
	}
...
func (h *httpDSLFunctions) Execute(filePath string) error {
	...
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL, debug, "-nc")
	if err != nil {
		return err
	}
...
func (h *httpPostBody) Execute(filePath string) error {
	...
	router.POST("/", func(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
		...
		if strings.EqualFold(r.Form.Get("username"), "test") && strings.EqualFold(r.Form.Get("password"), "vulmap") {
...
func (h *httpPostJSONBody) Execute(filePath string) error {
	...
	router.POST("/", func(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
		...
		if strings.EqualFold(obj.Username, "test") && strings.EqualFold(obj.Password, "vulmap") {
...
func (h *httpPostMultipartBody) Execute(filePath string) error {
	...
	router.POST("/", func(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
		...
		if strings.EqualFold(password[0], "vulmap") && strings.EqualFold(file[0].Filename, "username") {
...
func (h *httpRawDynamicExtractor) Execute(filePath string) error {
	...
	router.POST("/", func(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
		...
		if strings.EqualFold(r.Form.Get("testing"), "parameter") {
			fmt.Fprintf(w, "Token: 'vulmap'")
		}
	})
	router.GET("/", func(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
		if strings.EqualFold(r.URL.Query().Get("username"), "vulmap") {
...
func (h *httpRawWithParams) Execute(filePath string) error {
	...
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL+"/?key1=value1", debug)
	if err != nil {
		return err
	}
	if errx != nil {
...
func (h *httpRawPathTrailingSlash) Execute(filepath string) error {
	...
	_, err := testutils.RunVulmapTemplateAndGetResults(filepath, ts.URL, debug)
	if err != nil {
		return err
	}
	if routerErr != nil {
...
func (h *httpRawPayload) Execute(filePath string) error {
	...
	router.POST("/", func(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
		...
		if strings.EqualFold(r.Form.Get("username"), "test") && (strings.EqualFold(r.Form.Get("password"), "vulmap") || strings.EqualFold(r.Form.Get("password"), "guest")) {
...
func (h *httpRawUnsafePath) Execute(filepath string) error {
	...
	// hence it is done by parsing and matching paths from vulmap output with `-debug-req` flag
	// read template files
	...
	results, err := testutils.RunVulmapBinaryAndGetCombinedOutput(debug, []string{"-t", filepath, "-u", "scanme.sh", "-debug-req"})
	if err != nil {
		return err
	}
...
func (h *httpRawCookieReuse) Execute(filePath string) error {
	...
	router.POST("/", func(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
		...
		if strings.EqualFold(r.Form.Get("testing"), "parameter") {
			http.SetCookie(w, &http.Cookie{Name: "vulmap", Value: "test"})
		}
	})
	router.GET("/", func(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
		if err := r.ParseForm(); err != nil {
...
// 	defer ts.Close()
//
// 	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, "http://"+ts.URL, debug)
// 	if err != nil {
// 		return err
// 	}
// 	if routerErr != nil {
...
func (h *httpRequestSelfContained) Execute(filePath string) error {
	...
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, "", debug)
	if err != nil {
		return err
	}
...
func (h *httpRequestSelfContainedFileInput) Execute(filePath string) error {
	...
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, "", debug, "-V", "test="+FileLoc.Name())
	if err != nil {
		return err
	}
...
func (h *httpGetCaseInsensitiveCluster) Execute(filesPath string) error {
	...
	results, err := testutils.RunVulmapTemplateAndGetResults(files[0], ts.URL, debug, "-t", files[1])
	if err != nil {
		return err
	}
...
func (h *httpVariables) Execute(filePath string) error {
	...
	results, err = testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL, debug, "-var", "a1=failed")
	if err != nil {
		return err
	}
...
func (h *httpVariableDSLFunction) Execute(filePath string) error {
	results, err := testutils.RunVulmapBinaryAndGetCombinedOutput(debug, []string{"-t", filePath, "-u", "https://scanme.sh", "-debug-req"})
	if err != nil {
		return err
	}
...
func (h *customCLISNI) Execute(filePath string) error {
	...
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL, debug, "-sni", "test")
	if err != nil {
		return err
	}
	return expectResultsCount(results, 1)
...
func (h *httpRedirectMatchURL) Execute(filePath string) error {
	...
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL, debug, "-no-meta")
	if err != nil {
		return err
	}
...
func (h *annotationTimeout) Execute(filePath string) error {
	...
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL, debug, "-timeout", "1")
	if err != nil {
		return err
	}
	return expectResultsCount(results, 1)
...
func (h *customAttackType) Execute(filePath string) error {
	...
	_, err := testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL, debug, "-attack-type", "clusterbomb")
	if err != nil {
		return err
	}
...
func (h *scanAllIPS) Execute(filePath string) error {
	got, err := testutils.RunVulmapTemplateAndGetResults(filePath, "https://scanme.sh", debug, "-scan-all-ips", "-iv", "4")
	if err != nil {
		return err
	}
...
func (h *httpGetWithoutScheme) Execute(filePath string) error {
	got, err := testutils.RunVulmapTemplateAndGetResults(filePath, "scanme.sh", debug)
	if err != nil {
		return err
	}
	return expectResultsCount(got, 1)
...
func (h *httpCLBodyWithoutHeader) Execute(filePath string) error {
	...
	got, err := testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL, debug)
	if err != nil {
		return err
	}
	return expectResultsCount(got, 1)
...
func (h *ConstantWithCliVar) Execute(filePath string) error {
	...
	got, err := testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL, debug, "-V", "test=fromcli")
	if err != nil {
		return err
	}
	return expectResultsCount(got, 1)
...
func (h *matcherStatusTest) Execute(filePath string) error {
	...
	results, err := testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL, debug, "-ms")
	if err != nil {
		return err
	}
	return expectResultsCount(results, 1)
...
func (h *httpDisablePathAutomerge) Execute(filePath string) error {
	...
	ts := httptest.NewServer(router)
	defer ts.Close()
	got, err := testutils.RunVulmapTemplateAndGetResults(filePath, ts.URL+"/api/v1/user", debug)
	if err != nil {
		return err
	}
...
func (h *httpInteractshRequestsWithMCAnd) Execute(filePath string) error {
	got, err := testutils.RunVulmapTemplateAndGetResults(filePath, "honey.scanme.sh", debug)
	if err != nil {
		return err
	}
	return expectResultsCount(got, 1)
