...
func (d *dslVersionWarning) Execute(templatePath string) error {
	...
	defer ts.Close()
	results, err := testutils.RunVulmapArgsAndGetErrors(debug, defaultDSLEnvs, "-t", templatePath, "-target", ts.URL, "-v")
	if err != nil {
		return err
	}
...
func (d *dslShowVersionWarning) Execute(templatePath string) error {
	...
	ts := httptest.NewServer(router)
	defer ts.Close()
	results, err := testutils.RunVulmapArgsAndGetErrors(debug, append(defaultDSLEnvs, "SHOW_DSL_ERRORS=true"), "-t", templatePath, "-target", ts.URL, "-v")
	if err != nil {
		return err
	}
