...
func (h *remoteTemplateList) Execute(templateList string) error {
	...
	results, err := testutils.RunVulmapBareArgsAndGetResults(debug, nil, "-target", ts.URL, "-template-url", ts.URL+"/template_list", "-config", "test-config.yaml")
	if err != nil {
		return err
	}
...
func (h *excludedTemplate) Execute(templateList string) error {
	...
	results, err := testutils.RunVulmapBareArgsAndGetResults(debug, nil, "-target", ts.URL, "-t", templateList, "-include-templates", templateList)
	if err != nil {
		return err
	}
...
func (h *remoteTemplateListNotAllowed) Execute(templateList string) error {
	...
	_, err := testutils.RunVulmapBareArgsAndGetResults(debug, nil, "-target", ts.URL, "-template-url", ts.URL+"/template_list")
	if err == nil {
...
func (h *remoteWorkflowList) Execute(workflowList string) error {
	...
	results, err := testutils.RunVulmapBareArgsAndGetResults(debug, nil, "-target", ts.URL, "-workflow-url", ts.URL+"/workflow_list", "-config", "test-config.yaml")
	if err != nil {
		return err
	}
...
func (h *nonExistentTemplateList) Execute(nonExistingTemplateList string) error {
	...
	_, err := testutils.RunVulmapBareArgsAndGetResults(debug, nil, "-target", ts.URL, "-template-url", ts.URL+"/404")
	if err == nil {
...
func (h *nonExistentWorkflowList) Execute(nonExistingWorkflowList string) error {
	...
	_, err := testutils.RunVulmapBareArgsAndGetResults(debug, nil, "-target", ts.URL, "-workflow-url", ts.URL+"/404")
	if err == nil {
		return fmt.Errorf("expected error for nonexisting workflow url")
	}
...
func (h *loadTemplateWithID) Execute(nooop string) error {
	results, err := testutils.RunVulmapBareArgsAndGetResults(debug, nil, "-target", "scanme.sh", "-id", "self-signed-ssl")
	if err != nil {
