...
var (
	...
	mainVulmapBinary = flag.String("main", "", "Main Branch Vulmap Binary")
	devVulmapBinary  = flag.String("dev", "", "Dev Branch Vulmap Binary")
	testcases        = flag.String("testcases", "", "Test cases file for vulmap functional tests")
)
...
func runIndividualTestCase(testcase string, debug bool) error {
	...
	mainOutput, err := testutils.RunVulmapBinaryAndGetLoadedTemplates(*mainVulmapBinary, debug, finalArgs)
	if err != nil {
		return errors.Wrap(err, "could not run vulmap main test")
	}
	devOutput, err := testutils.RunVulmapBinaryAndGetLoadedTemplates(*devVulmapBinary, debug, finalArgs)
	if err != nil {
		return errors.Wrap(err, "could not run vulmap dev test")
	}
