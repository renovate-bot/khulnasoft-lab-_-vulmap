...
import (
	...
	"github.com/khulnasoft-lab/vulmap/pkg/protocols/common/uncover"
	...
	"github.com/khulnasoft-lab/vulmap/pkg/utils/monitor"
...
func main() {
	...
	vulmapRunner, err := runner.New(options)
	if err != nil {
	...
	go func() {
		for range c {
			...
			if options.ShouldSaveResume() {
				...
				err := vulmapRunner.SaveResumeConfig(resumeFileName)
				if err != nil {
	...
	if err := vulmapRunner.RunEnumeration(); err != nil {
		if options.Validate {
			gologger.Fatal().Msgf("Could not validate templates: %s\n", err)
		} else {
			gologger.Fatal().Msgf("Could not run vulmap: %s\n", err)
		}
	}
	vulmapRunner.Close()
...
func readConfig() *goflags.FlagSet {
	...
	// when true updates vulmap binary to latest version
	var updateVulmapBinary bool

	flagSet := goflags.NewFlagSet()
	flagSet.CaseSensitive = true
	flagSet.SetDescription(`Vulmap is a fast, template based vulnerability scanner focusing
on extensive configurability, massive extensibility and ease of use.`)
	...
	*/
	...
	flagSet.CreateGroup("templates", "Templates",
		flagSet.BoolVarP(&options.NewTemplates, "new-templates", "nt", false, "run only new templates added in latest vulmap-templates release"),
		...
		flagSet.BoolVar(&options.Validate, "validate", false, "validate the passed templates to vulmap"),
		...
		flagSet.StringSliceVarConfigOnly(&options.RemoteTemplateDomainList, "remote-template-domain", []string{"templates.vulmap.sh"}, "allowed domain list to load remote templates from"),
		flagSet.BoolVar(&options.SignTemplates, "sign", false, "signs the templates with the private key defined in VULMAP_SIGNATURE_PRIVATE_KEY env variable"),
	)
	...
	flagSet.CreateGroup("output", "Output",
		...
		flagSet.BoolVarP(&options.StoreResponse, "store-resp", "sresp", false, "store all request/response passed through vulmap to output directory"),
		flagSet.StringVarP(&options.StoreResponseDir, "store-resp-dir", "srd", runner.DefaultDumpTrafficOutputFolder, "store all request/response passed through vulmap to custom directory"),
		...
		flagSet.StringVarP(&options.ReportingDB, "report-db", "rdb", "", "vulmap reporting database (always use this to persist report data)"),
	...
	flagSet.CreateGroup("configs", "Configurations",
		flagSet.StringVar(&cfgFile, "config", "", "path to the vulmap configuration file"),
		...
		flagSet.StringVarP(&options.ReportingConfig, "report-config", "rc", "", "vulmap reporting module configuration file"), // TODO merge into the config file or rename to issue-tracking
		...
		flagSet.StringVarP(&options.ResolversFile, "resolvers", "r", "", "file containing resolver list for vulmap"),
		...
		flagSet.CallbackVar(resetCallback, "reset", "reset removes all vulmap configuration and data files (including vulmap-templates)"),
	...
	flagSet.CreateGroup("headless", "Headless",
		...
		flagSet.BoolVarP(&options.UseInstalledChrome, "system-chrome", "sc", false, "use local installed Chrome browser instead of vulmap installed"),
	...
	flagSet.CreateGroup("debug", "Debug",
		...
		flagSet.CallbackVar(printVersion, "version", "show vulmap version"),
		flagSet.BoolVarP(&options.HangMonitor, "hang-monitor", "hm", false, "enable vulmap hang monitoring"),
		flagSet.BoolVarP(&options.Verbose, "verbose", "v", false, "show verbose output"),
		flagSet.StringVar(&memProfile, "profile-mem", "", "optional vulmap memory profile dump file"),
		...
		flagSet.CallbackVarP(printTemplateVersion, "templates-version", "tv", "shows the version of the installed vulmap-templates"),
		flagSet.BoolVarP(&options.HealthCheck, "health-check", "hc", false, "run diagnostic check up"),
	)

	flagSet.CreateGroup("update", "Update",
		flagSet.BoolVarP(&updateVulmapBinary, "update", "up", false, "update vulmap engine to the latest released version"),
		flagSet.BoolVarP(&options.UpdateTemplates, "update-templates", "ut", false, "update vulmap-templates to latest released version"),
		flagSet.StringVarP(&options.NewTemplatesDirectory, "update-template-dir", "ud", "", "custom directory to install / update vulmap-templates"),
		flagSet.CallbackVarP(disableUpdatesCallback, "disable-update-check", "duc", "disable automatic vulmap/templates update check"),
	)
	...
	flagSet.CreateGroup("stats", "Statistics",
		...
		flagSet.IntVarP(&options.MetricsPort, "metrics-port", "mp", 9092, "port to expose vulmap metrics on"),
	)

	flagSet.CreateGroup("cloud", "Cloud",
		flagSet.BoolVar(&options.Cloud, "cloud", false, "run scan on vulmap cloud"),
...
Run vulmap on single host:
	$ vulmap -target example.com

Run vulmap with specific template directories:
	$ vulmap -target example.com -t http/cves/ -t ssl

Run vulmap against a list of hosts:
	$ vulmap -list hosts.txt

Run vulmap with a JSON output:
	$ vulmap -target example.com -json-export output.json

Run vulmap with sorted Markdown outputs (with environment variables):
	$ MARKDOWN_EXPORT_SORT_MODE=template vulmap -target example.com -markdown-export vulmap_report/

Additional documentation is available at: https://docs.vulmap.sh/getting-started/running
	`)

	// vulmap has multiple migrations
	...
	if updateVulmapBinary {
		runner.VulmapToolUpdateCallback()
	}

	if options.LeaveDefaultPorts {
		http.LeaveDefaultPorts = true
	}
	if customConfigDir := os.Getenv(config.VulmapConfigDirEnv); customConfigDir != "" {
...
// printVersion prints the vulmap version and exits.
func printVersion() {
	gologger.Info().Msgf("Vulmap Engine Version: %s", config.Version)
	gologger.Info().Msgf("Vulmap Config Directory: %s", config.DefaultConfig.GetConfigDir())
	gologger.Info().Msgf("Vulmap Cache Directory: %s", config.DefaultConfig.GetCacheDir()) // cache dir contains resume files
	os.Exit(0)
}

// printTemplateVersion prints the vulmap template version and exits.
func printTemplateVersion() {
	cfg := config.DefaultConfig
	gologger.Info().Msgf("Public vulmap-templates version: %s (%s)\n", cfg.TemplateVersion, cfg.TemplatesDirectory)
...
func resetCallback() {
	warning := fmt.Sprintf(`
Using '-reset' will delete all vulmap configurations files and all vulmap-templates

Following files will be deleted:
1. All Config + Resumes files at %v
2. All vulmap-templates at %v

Note: Make sure you have backup of your custom vulmap-templates before proceeding
...
`, config.DefaultConfig.GetConfigDir(), config.DefaultConfig.TemplatesDirectory)
	...
	gologger.Info().Msgf("Successfully deleted all vulmap configurations files and vulmap-templates")
	os.Exit(0)
}
